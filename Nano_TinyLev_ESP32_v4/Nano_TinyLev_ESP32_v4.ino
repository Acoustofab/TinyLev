...
// ---------------------------------------------------------------------
// The ESP32 creates its own WiFi network (access point mode) and hosts
// the control page itself — no separate file, no Live Server, no
// browser BLE permissions to deal with. Connect your laptop's WiFi to
// the network below, then open http://192.168.4.1 in any browser.
//
// Downside of AP mode: your laptop won't have internet access while
// connected to it (normal limitation of a device-hosted WiFi network
// with no internet uplink of its own). If you'd rather have the ESP32
// join your existing home/lab WiFi instead (so you keep internet
// access and can reach it by a normal local address), that's a
// straightforward change — ask and I'll switch it to station mode.
#define WIFI_AP_SSID     "TinyLev"
#define WIFI_AP_PASSWORD "levitate123" // must be 8+ characters for WPA2; change freely

static WebServer webServer(80);

// Compact instrument-style control page, served directly from flash.
// Sends commands via simple GET requests (?key=w / ?key=s / ?key=%1B)
// rather than Web Bluetooth — this is just normal HTTP, so it works
// the same in every browser with zero special permissions.
...
<div class="box">
  ...
  <div class="keys">
    <div class="k" id="up"><b>Phase up</b><small>W / Page Up</small></div>
    <div class="k" id="down"><b>Phase down</b><small>S / Page Down</small></div>
    <div class="k r" id="reset"><b>Reset to frame 0</b><small>Esc</small></div>
  </div>
  <div class="fn">Keyboard works once you've clicked anywhere on this page. Buttons work by tap/click too.</div>
</div>
<script>
  ...
  const sendUp    = () => send('w', upEl);
  const sendDown  = () => send('s', downEl);
  const sendReset = () => send('\x1b', resetEl);

  upEl.addEventListener('click', sendUp);
  downEl.addEventListener('click', sendDown);
  resetEl.addEventListener('click', sendReset);

  window.addEventListener('keydown', (e) => {
    if (e.repeat) return;
    switch (e.key) {
      case 'w': case 'W': case 'PageUp':   e.preventDefault(); sendUp();    break;
      case 's': case 'S': case 'PageDown': e.preventDefault(); sendDown();  break;
